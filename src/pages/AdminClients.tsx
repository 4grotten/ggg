              ...
              >
                <div className="bg-card rounded-2xl border border-border/50 p-4 space-y-3">
                  <div className="flex items-center justify-between">
                    <span className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">{t("admin.clients.filters")}</span>
                    {activeFiltersCount > 0 && (
                      <button onClick={clearFilters} className="text-xs text-primary flex items-center gap-1">
                        <X className="w-3 h-3" /> {t("admin.clients.reset")}
                      </button>
                    )}
                  </div>
                  ...
                  {/* Role */}
                  <div>
                    <p className="text-[11px] text-muted-foreground mb-1.5 font-medium">{t("admin.clients.roleLabel")}</p>
                    <div className="flex gap-1.5 flex-wrap">
                      <FilterChip label={t("admin.clients.all")} active={roleFilter === "all"} onClick={() => setRoleFilter("all")} />
                      <FilterChip label="Root" active={roleFilter === "root"} onClick={() => setRoleFilter("root")} />
                      <FilterChip label={t("admin.clients.adminRole")} active={roleFilter === "admin"} onClick={() => setRoleFilter("admin")} />
                      <FilterChip label={t("admin.clients.moderatorRole")} active={roleFilter === "moderator"} onClick={() => setRoleFilter("moderator")} />
                      <FilterChip label={t("admin.clients.userRole")} active={roleFilter === "user"} onClick={() => setRoleFilter("user")} />
                    </div>
                  </div>

                  {/* Verification */}
                  <div>
                    <p className="text-[11px] text-muted-foreground mb-1.5 font-medium">{t("admin.clients.verificationLabel")}</p>
                    <div className="flex gap-1.5 flex-wrap">
                      <FilterChip label={t("admin.clients.all")} active={verificationFilter === "all"} onClick={() => setVerificationFilter("all")} />
                      <FilterChip label={`✅ ${t("admin.clients.verified")}`} active={verificationFilter === "verified"} onClick={() => setVerificationFilter("verified")} />
                      <FilterChip label={`⏳ ${t("admin.clients.notVerified")}`} active={verificationFilter === "unverified"} onClick={() => setVerificationFilter("unverified")} />
                    </div>
                  </div>

                  {/* Assets */}
                  <div>
                    <p className="text-[11px] text-muted-foreground mb-1.5 font-medium">{t("admin.clients.assetsLabel")}</p>
                    <div className="flex gap-1.5 flex-wrap">
                      <FilterChip label={t("admin.clients.all")} active={assetsFilter === "all"} onClick={() => setAssetsFilter("all")} />
                      <FilterChip label={`💳 ${t("admin.clients.hasCards")}`} active={assetsFilter === "has_cards"} onClick={() => setAssetsFilter("has_cards")} />
                      <FilterChip label={`🏦 ${t("admin.clients.hasAccounts")}`} active={assetsFilter === "has_accounts"} onClick={() => setAssetsFilter("has_accounts")} />
                      <FilterChip label={`₿ ${t("admin.clients.hasCrypto")}`} active={assetsFilter === "has_crypto"} onClick={() => setAssetsFilter("has_crypto")} />
                    </div>
                  </div>
                </div>
              </motion.div>
          ...
          {clientsError ? (
            <div className="flex flex-col items-center justify-center py-16 space-y-4">
              ...
              <div className="text-center space-y-1">
                <p className="font-semibold text-sm">{t("admin.clients.loadError")}</p>
                <p className="text-xs text-muted-foreground max-w-[250px]">{t("admin.clients.serverUnavailable")}</p>
              </div>
              <Button variant="outline" size="sm" onClick={() => refetchClients()} className="rounded-xl gap-2">
                <RefreshCw className="w-4 h-4" />
                {t("admin.clients.retry")}
              </Button>
            </div>
          ) : clientsLoading ? (
                ...
                >
                  ...
                  <div className="p-4">
                    ...
                    <div className="flex items-start gap-3 mb-3">
                      ...
                      <div className="flex-1 min-w-0">
                        <h4 className="font-semibold text-sm truncate">{(client.full_name && client.full_name !== "None None" && client.full_name.trim()) ? client.full_name : t("admin.clients.noName")}</h4>
                    ...
                    <div className="flex items-center gap-2 flex-wrap mb-3">
                      ...
                      {client.created_at && (
                        <div className="flex items-center gap-2 bg-muted/40 rounded-lg px-2.5 py-1.5 w-fit">
                          ...
                          <span className="text-xs font-medium text-muted-foreground">{new Date(client.created_at).toLocaleDateString(undefined, { day: '2-digit', month: '2-digit', year: 'numeric' })}</span>
                        </div>
                      )}
                    </div>
                    ...
                    <div className="flex items-center gap-1.5 flex-wrap mb-3">
                      {client.is_verified ? (
                        <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 text-[10px] font-medium">
                          <CheckCircle className="w-3 h-3" /> {t("admin.clients.verified")}
                        </span>
                      ) : (
                    ...
                    <div className="grid grid-cols-3 gap-2 mb-2">
                      <div className="bg-muted/30 rounded-xl px-2.5 py-2 text-center">
                        <div className="flex items-center justify-center gap-1 mb-0.5">
                          ...
                          <span className="text-[10px] text-muted-foreground">{t("admin.clients.cardsLabel")}</span>
                        </div>
                      ...
                      <div className="bg-muted/30 rounded-xl px-2.5 py-2 text-center">
                        <div className="flex items-center justify-center gap-1 mb-0.5">
                          ...
                          <span className="text-[10px] text-muted-foreground">{t("admin.clients.accountsLabel")}</span>
                        </div>
                      ...
                      <div className="bg-muted/30 rounded-xl px-2.5 py-2 text-center">
                        <div className="flex items-center justify-center gap-1 mb-0.5">
                          ...
                          <span className="text-[10px] text-muted-foreground">{t("admin.clients.cryptoLabel")}</span>
                        </div>
                    ...
                    <div className="flex items-center gap-3 pt-2 border-t border-border/30">
                      <div className="flex-1">
                        <p className="text-[10px] text-muted-foreground">{t("admin.clients.cardsAndAccount")}</p>
                      ...
                      <div className="flex-1">
                        <p className="text-[10px] text-muted-foreground">{t("admin.clients.cryptoLabel")}</p>
                        <p className="text-sm font-bold text-foreground">
            ...
            <div className="flex flex-col items-center py-16 text-muted-foreground">
              ...
              <p className="text-sm font-medium">{searchQuery || activeFiltersCount > 0 ? t("admin.clients.nothingFound") : t("admin.clients.noClients")}</p>
              {activeFiltersCount > 0 && (
                <Button variant="link" onClick={clearFilters} className="mt-1 text-xs">{t("admin.clients.resetFilters")}</Button>
              )}
            </div>
          )}
        </div>
