            ...
            <div className="relative bg-muted/70 dark:bg-card/70 backdrop-blur-xl rounded-3xl p-5 border border-border/50 overflow-hidden">
              ...
              <div className="space-y-3 mb-5">
                {selectedTariff.includes.map((item, idx) => (
                  <motion.div 
                    key={`${selectedTariff.id}-include-${idx}`}
                ...
                <>
                  ...
                  <div className="space-y-3">
                    {selectedTariff.alsoIncludes.map((item, idx) => (
                      <motion.div 
                        key={`${selectedTariff.id}-also-${idx}`}
                        initial={{ opacity: 0, y: -10 }}
                        animate={{ opacity: 1, y: 0 }}
                        transition={{ 
                          duration: 0.3, 
                          delay: (selectedTariff.includes.length + idx) * 0.08,
                          ease: "easeOut"
                        }}
                        className="flex items-start gap-3"
                      >
                        <motion.div 
                          ...
                          transition={{ 
                            duration: 0.2, 
                            delay: (selectedTariff.includes.length + idx) * 0.08 + 0.1,
                            type: "spring",
                            stiffness: 400
                          }}
                        >
